"""
Nika Visa Blog Bot — Step 4: Image Generation
...
async def generate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Build preview
        outline_text = "\n".join([f"  • {h['text']}" for h in post.get("outline", [])[8]])
        tags_text = " ".join([f"#{t}" for t in post.get("tags", [])])
        import re
        content_preview = re.sub(r'<[^>]+>', '', post["content_html"])[:300]
        image_status = "✅ Image generated" if image_url else "⚠️ Image failed (will use default)"
        ...
        # Send image + preview together if image was generated
        if image_url:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        ...
        if not post:
            await query.edit_message_text(f"⚠️ Post `{slug}` not found. Try generating again.")
            return
        ...
        PENDING_POSTS.pop(slug, None)
        await query.edit_message_caption(f"❌ Rejected. Send `/generate <topic>` to try again.", parse_mode="Markdown")
